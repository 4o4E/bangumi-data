...
服务首次启动后会在后台读取官方 Archive：只导入动画、游戏、角色和完整作品—角色关系，过滤儿童向条目；作品没有中文名时使用原名，NSFW 作品也参与关系和热度计算。熟悉关系需要同时通过作品内角色相对热度、同类型全站角色绝对热度、同类型全站作品收藏热度和同期作品横向热度四层自适应分布；任一层落入长尾都会被排除。全站角色与作品热度使用四簇细分并只保留最高热度层；同期层只在最低簇与中间簇的几何中心至少相差两倍时剔除最低簇，连续分布完整保留。TV 动画按季度、其他动画按载体和年份、游戏按年份比较，样本不足时扩展相邻时期。算法不使用固定作品名单、固定收藏数阈值，也不直接按主角/配角截断；版本保存在关系记录中，升级后会对当前数据代重新计算。

Archive 的作品发行日期、载体及收藏状态 `wish`、`done`、`doing`、`on_hold`、`dropped` 会原样保存，并随作品响应返回。缺少有效日期的作品只有全局核心层可以继续通过。诊断接口可以对照角色收藏数、作品总收藏、作品内排名和入选原因，区分冷门作品的头部角色、同期作品长尾与热门作品中误带出的长尾角色。旧数据代升级后会自动重读一次当前 Archive 补齐这些字段，但会复用已有角色和作品详情补充进度，不采集历史快照。

最终候选再通过 Bangumi v0 补齐性别、名称、别名和图片；没有中文角色名时使用原名。NSFW 作品与普通作品一样保留 Bangumi 返回的封面，角色本身仍按角色级 NSFW 标记排除。作品封面以 `imageStatus` 的 `PENDING`、`AVAILABLE`、`MISSING_UPSTREAM`、`NOT_FOUND`、`FETCH_FAILED` 区分待补充、可用、上游无图、404 和请求失败：上游无图与 404 每七天复查，请求失败在下一轮同步重试。Archive 基线与详情补充均按批提交，并通过来源摘要和补充状态断点续采；首次同步得到第一批有效角色后即可查询，作品信息会继续在后台逐批完善。
