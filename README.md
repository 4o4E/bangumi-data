...
服务首次启动后会在后台读取官方 Archive：只导入动画、游戏、角色和完整作品—角色关系，过滤 NSFW、儿童向及缺少中文作品名的条目。熟悉关系需要同时通过作品内角色相对热度、同类型全站角色绝对热度和同类型全站作品收藏热度三层自适应分布；任一层落入长尾都会被排除。算法不使用固定作品名单、固定收藏数阈值，也不直接按主角/配角截断；版本保存在关系记录中，升级后会对当前数据代重新计算。
